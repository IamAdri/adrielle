...
import GiveRatingAtItemPage from "./GiveRatingAtItemPage";

async function RatingAndReviewsFromAllUsers({ itemName, currentUser }) {
      ...
      <div className="mt-15">
        {getReviewsAndRatingsOfProduct.map((review) => {
          ...
          return (
            <div
              key={review.id}
              className="flex gap-15 items-start border-b border-coolgrey mb-15 w-150"
            >
              <span>{review.userName}</span>
              <div className="flex flex-col items-start gap-3">
                <StaticRatingStars rating={review.rating} />
                <p className="text-left w-95">{review.review}</p>
              </div>
            </div>
          );
        })}
      </div>
      <GiveRatingAtItemPage />
    </div>
  );
}
